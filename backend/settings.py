...
class _AzureAIAgentsSettings(BaseSettings):
    """Azure AI Agents Service settings for modern RAG + Web search integration"""
    model_config = SettingsConfigDict(
    ...
    bing_grounding_conn_id: Optional[str] = Field(None, env="BING_GROUNDING_CONN_ID")
    ai_search_conn_id: Optional[str] = Field(None, env="AI_SEARCH_CONN_ID")
    ai_search_index_name: Optional[str] = Field(None, env="AI_SEARCH_INDEX_NAME")
    response_timeout: float = 30.0
...
class _BaseSettings(BaseSettings):
    ...
    auth_enabled: bool = Field(default=os.environ.get("AUTH_REQUIRED", "false").lower() == "true")
...
class _AppSettings(BaseModel):
    ...
    azure_ai_agents: Optional[_AzureAIAgentsSettings] = None
    ...
    @model_validator(mode="after")
    def set_azure_ai_agents_settings(self) -> Self:
        try:
            self.azure_ai_agents = _AzureAIAgentsSettings()
            
        except ValidationError:
            self.azure_ai_agents = None
