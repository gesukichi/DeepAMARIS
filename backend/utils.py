    ...
    if len(chatCompletion.choices) > 0:
        ...
        if message:
            ...
            if hasattr(message, "context"):
                message_obj["context"] = json.dumps(message.context)
    ...
    if len(chatCompletionChunk.choices) > 0:
        ...
        if delta:
            if hasattr(delta, "context"):
                # Don't send 'tool' role messages to frontend - they cause OpenAI API errors
                # Include context in assistant message instead
                messageObj = {"role": "assistant", "content": "", "context": json.dumps(delta.context)}
            ...
            if delta.role == "assistant" and hasattr(delta, "context"):
                ...
                return response_obj
            if delta.tool_calls:
                # Send tool_calls as assistant message with tool_calls field
                # Frontend will handle this without creating 'tool' role messages
                messageObj = {
                    "role": "assistant",
                    "content": "",
                    "tool_calls": {
    ...
    try:
        ...
        if citations_field_name in chatCompletion:
            ...
            if not assistant_message:
                ...
                messages.append(assistant_message)
            assistant_message["context"] = json.dumps(citation_content)
