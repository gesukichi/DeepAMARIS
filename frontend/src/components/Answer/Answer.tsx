...
export const Answer = ({ answer, onCitationClicked, onExectResultClicked }: Props) => {
  ...
  // Modern RAG引用情報解析関数
  const parseCitations = (toolMessage: any) => {
    try {
      if (typeof toolMessage === 'string') {
        const content = JSON.parse(toolMessage)
        return content.citations || []
      } else if (toolMessage && typeof toolMessage === 'object') {
  ...
  // API Citation型をCitationDisplay Citation型に変換
  const convertToCitationDisplayFormat = (apiCitations: Citation[]) => {
    return apiCitations.map(citation => ({
      type: (citation.metadata ? 'internal_search' : 'web_search') as 'web_search' | 'internal_search' | 'error',
      source: citation.metadata || 'web_search',
      title: citation.title || 'Citation',
      url: citation.url || undefined,
      query: citation.content,
      index: citation.id
    }))
  }
  ...
  return (
    <>
      <Stack className={styles.answerContainer} tabIndex={0}>
        ...
        {/* Modern RAG引用情報表示 - citationsプロパティから表示 */}
        {answer.citations && answer.citations.length > 0 && (
          <CitationDisplay citations={convertToCitationDisplayFormat(answer.citations)} />
        )}
