...
import { CommandBarButton, IconButton, Dialog, DialogType, Stack, Toggle, Text } from '@fluentui/react'
...
import ActivarchLogo2 from '../../assets/amaris_kari3.svg'
...
const modeOptions: ModeOption[] = [
  {
    id: 'normal',
    title: 'ノーマルチャット',
    description: 'Azure OpenAI による標準的な会話',
...
const Chat = () => {
  ...
  useEffect(() => {
    if (!appStateContext?.state.isLoading) {
      setLogo(ui?.chat_logo || ui?.logo || ActivarchLogo2)
    }
  ...
  const processResultMessage = (resultMessage: ChatMessage, userMessage: ChatMessage, conversationId?: string) => {
    ...
    if (resultMessage.role === TOOL) {
      ...
      // Modern RAG citation情報をassistantMessageに統合
      try {
        const toolContent = typeof resultMessage.content === 'string' 
          ? JSON.parse(resultMessage.content) 
          : resultMessage.content
        
        if (toolContent.citations && Array.isArray(toolContent.citations)) {
          // assistantMessageにcitation情報を追加
          if (!assistantMessage.context) {
            assistantMessage.context = JSON.stringify({ citations: toolContent.citations })
          } else {
            const existingContext = JSON.parse(assistantMessage.context)
            existingContext.citations = toolContent.citations
            assistantMessage.context = JSON.stringify(existingContext)
          }
        }
      } catch (e) {
        console.warn('Failed to parse tool message for citations:', e)
      }
    }
  ...
  const makeApiRequestWithCosmosDB = async (question: ChatMessage["content"], conversationId?: string) => {
    ...
    if (conversationId) {
      ...
      if (!conversation) {
        ...
        request = {
          messages: [userMessage].filter(answer => answer.role !== ERROR && answer.role !== TOOL)
        }
  ...
  const parseCitationFromMessage = (message: ChatMessage) => {
    // Modern RAG: assistantメッセージのcontextからcitation情報を取得
    ...
    // 従来のtoolメッセージからのcitation取得（後方互換性）
                      ...
                      <div className={styles.chatMessageGpt}>
                        {typeof answer.content === "string" && <Answer
                          answer={{
                            ...
                            citations: parseCitationFromMessage(answer),  // 現在のmessageから取得
