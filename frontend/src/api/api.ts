...
const stripToolMessages = (messages?: ChatMessage[]): ChatMessage[] => {
...
export async function modernRagWebConversationApi(options: ConversationRequest, abortSignal: AbortSignal): Promise<Response> {
  const response = await fetch('/conversation/modern-rag-web', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
